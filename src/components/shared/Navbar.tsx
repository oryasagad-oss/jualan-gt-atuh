...
export const Navbar: React.FC<NavbarProps> = ({ settings }) => {
  ...
  return (
    <header className={`sticky top-0 z-40 w-full transition-all duration-300 ${
      scrolled 
        ? 'bg-white/90 dark:bg-[#080c14]/90 backdrop-blur-md border-b border-slate-200 dark:border-gt-cardBorder shadow-md dark:shadow-lg dark:shadow-black/40' 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between">
        ...
        >
          {/* Pixel Diamond Logo */}
          <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl bg-gradient-to-br from-amber-400 via-yellow-500 to-amber-600 p-0.5 shadow-pixel-amber group-hover:scale-105 transition-transform flex items-center justify-center">
            <div className="w-full h-full bg-slate-900 dark:bg-[#111827] rounded-[10px] flex items-center justify-center relative overflow-hidden">
              <span className="text-xl sm:text-2xl select-none group-hover:animate-bounce">💎</span>
            </div>
          </div>
          
          <div>
            <div className="flex items-center gap-2">
              <span className="font-pixelHeading text-base sm:text-lg tracking-wider text-slate-900 dark:text-white font-extrabold group-hover:text-amber-500 dark:group-hover:text-amber-400 transition-colors">
                WICSTORE
              </span>
        ...
        <div className="flex items-center gap-2 sm:gap-2.5">
          ...
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-semibold rounded-lg border transition-all bg-slate-100 hover:bg-slate-200 text-slate-700 border-slate-300 dark:bg-gt-card dark:hover:bg-gt-cardHover dark:text-amber-300 dark:border-gt-cardBorder"
          >
          ...
          <button
            ...
            className="p-2 text-slate-600 hover:text-amber-600 dark:text-gray-400 dark:hover:text-amber-400 bg-slate-100 hover:bg-slate-200 dark:bg-gt-card dark:hover:bg-gt-cardHover border border-slate-300 dark:border-gt-cardBorder rounded-lg transition-colors"
          >
            {isDark ? (
          ...
          {/* Primary CTA: Discord (Replaces WhatsApp) */}
          <a
            href={`https://discord.com/users/${settings.discordId}`}
            target="_blank"
            rel="noopener noreferrer"
            onClick={() => sounds.playCoinSound()}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white bg-[#5865F2] hover:bg-[#4752C4] rounded-lg shadow-pixel-blue transition-all"
          >
            {/* Discord Icon SVG */}
          ...
          <Link
            ...
            className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-amber-700 hover:text-amber-900 dark:text-amber-300 dark:hover:text-white bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 rounded-lg transition-all"
          >
