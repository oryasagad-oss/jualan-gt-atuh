...
import { formatRupiah, formatLocks, generateDiscordProfileUrl, generateDiscordOrderMessage } from '../../utils/formatters';
...
export const AccountModal: React.FC<AccountModalProps> = ({
  ...
  const handleDiscordOrder = () => {
    ...
    setOrderCopied(true);
    setTimeout(() => setOrderCopied(false), 3500);

    // Open Discord Profile in a new tab
    window.open(generateDiscordProfileUrl(settings.discordId), '_blank', 'noopener,noreferrer');
  };
        ...
        <div className="p-4 sm:p-5 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <button
              ...
              title={`Discord ID: ${settings.discordId}`}
              className="px-3.5 py-2.5 rounded-xl bg-white hover:bg-slate-100 dark:bg-slate-900 dark:hover:bg-slate-800 text-slate-700 dark:text-gray-300 text-xs border border-slate-300 dark:border-slate-800 flex items-center justify-center gap-1.5 transition-colors w-1/2 sm:w-auto"
            >
              {copiedDiscord ? <Check className="w-4 h-4 text-emerald-500" /> : <Copy className="w-4 h-4" />}
              <span>{copiedDiscord ? t.modal.btnCopied : `ID: ${settings.discordId}`}</span>
            </button>
          </div>
          ...
          {account.isAvailable ? (
            <button
              ...
              className="w-full sm:w-auto px-6 py-3 rounded-xl bg-[#5865F2] hover:bg-[#4752C4] text-white font-bold text-sm flex items-center justify-center gap-2 shadow-pixel-blue transition-all active:translate-y-0.5"
            >
